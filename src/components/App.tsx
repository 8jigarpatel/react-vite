...
function App() {
  ...
  return (
    <div className="container min-w-full h-screen flex dark:text-white dark:bg-slate-600">
      <div className={`absolute transition-all ${isMobile ? 'w-screen' : 'w-64'}`}>
        <div className={`transition-all ${sidebarVisible ? '' : 'hidden'}`}>
          <Sidebar />
        </div>
        <button type="button" className={`absolute top-6 left-3 ${isMobile ? '' : 'hidden'}`} onClick={toggleSidbar}><FaBars /></button>
      </div>
